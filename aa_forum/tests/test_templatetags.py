...
from aa_forum.constants import PACKAGE_NAME
...
class TestForumVersionedStatic(TestCase):
    ...
    def test_versioned_static(self):
        ...
        expected_static_css_src = (
            f'/static/{PACKAGE_NAME}/css/aa-forum.min.css?v={context["version"]}'
        )
        expected_static_css_src_integrity = calculate_integrity_hash(
            "css/aa-forum.min.css"
        )
        expected_static_js_src = (
            f'/static/{PACKAGE_NAME}/javascript/aa-forum.min.js?v={context["version"]}'
        )
    ...
    def test_invalid_file_type(self) -> None:
        ...
        template_to_render = Template(
            template_string=(
                "{% load aa_forum %}" "{% aa_forum_static 'invalid/invalid.txt' %}"
            )
        )
